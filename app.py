...
def profile(username = None):
	...
	if username:
		...
		stream = current_user.get_posts().order_by(models.Post.timestamp.desc())
		user = current_user
	if username:
		template = 'profile.html'
...
def edit_post(post_id):
		form = forms.EditForm()
		if form.validate_on_submit():
			query=models.Post.update(content = form.content.data.strip()).where(models.Post.id == post_id)
			query.execute()
